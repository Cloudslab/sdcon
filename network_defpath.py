...
#####################################################
# Broadcasting and flooding rules to forward everywhere
#####################################################
# Add port-base flooding rule from up-port to down-port. This is only used for unknown-destination.
def set_broadcast_rules(switch, inport, broadcast_ports):
    add_flow_path(switch, broadcast_ports, network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH_BROADCAST,\
        match_inport=inport, match_dst_mac="ff:ff:ff:ff:ff:ff", \
        table_id = TABLE_ID_BASE)

def add_path_broadcast_common(topo, switch_id):
    upports, downports = get_up_down_ports(topo, switch_id)
    # From up: broadcast ONLY to downports (all), NO upports
    for inport in upports:
        set_broadcast_rules(switch_id, inport, downports)
    
    # From down: broadcast to ALL up ports, plus ALL the other downports.
    in_out = get_default_path_port_pair(all_inports= downports, all_outports = upports)
    for inport in downports:
        outports = list(downports)
        outports.remove(inport)
        #outports.append( in_out[inport] ) # Only to selected up-port
        outports += upports   # All up-ports
        set_broadcast_rules(switch_id, inport, outports)

def add_path_broadcast_core(topo, switch_id):
    all_ports = topo.get_all_ports(switch_id)
    for inport in all_ports:
        outports = list(all_ports)
        outports.remove(inport)
        set_broadcast_rules(switch_id, inport, outports)

def add_path_flood(topo, switch_id):
    upports, downports = get_up_down_ports(topo, switch_id)
    print "Debug: (%s) up/down="%(switch_id), upports, downports
    # in = downport, out = upports
    for inport in upports: 
        add_flow_path(switch_id, downports, network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH,\
            match_inport=inport,  \
            table_id = TABLE_ID_BASE)
    if len(upports) == 0 and len(downports) != 0:
        add_flow_path(switch_id, downports, network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH,\
            table_id = TABLE_ID_BASE)

#####################################################
# Self-learning mode: learn MAC address incoming from 'down' ports
#####################################################
def __run_ssh_cmd(switch_id, cmd):
    switch_ip = sdcon_config.switch_dpid_to_ip(switch_id)
    ssh = "./cmd-run-ssh.sh"
    server = "pi@" +switch_ip    
    print ssh, server, cmd
    print subprocess.check_output([ssh, server, cmd])

def __run_ssh_add_flow_command(switch_id, downport):
    switch_ip = sdcon_config.switch_dpid_to_ip(switch_id)
    cmd = '''sudo ovs-ofctl -O OpenFlow13 add-flow ovsbr0 'table='''+str(0)+\
        ''' priority=''' +str(network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH_LEARN) +\
        ''' in_port=''' +str(downport)+\
        ''' actions=learn(table='''+str(TABLE_ID_HOST)+\
        ''',idle_timeout=120,'''+\
        ''',priority='''+str(network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH_FOR_HOST)+\
        ''',NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[],output:NXM_OF_IN_PORT[]), resubmit(,'''+str(TABLE_ID_BASE) +''')\' '''
    __run_ssh_cmd(switch_id, cmd)

def __run_ssh_remove_flow_command(switch_id, downport):
    cmd ='''sudo ovs-ofctl del-flows ovsbr0 'table=''' +str(0)+ \
        ''' priority='''+str(network_manager.ODL_FLOW_PRIORITY_DEFAULT_PATH_LEARN)+''' in_port='''+downport+'''' --strict'''
    __run_ssh_cmd(switch_id, cmd)

def add_rule_self_learn(topo, switch_id):
    if ENABLE_SELF_LEARN:
        upports, downports = get_up_down_ports(topo, switch_id)
        # Add self-learn rule for the packets coming from down-ports
        for downport in downports:
            __run_ssh_add_flow_command(switch_id, downport)

def del_all_rule_self_learn(topo):
    if ENABLE_SELF_LEARN:
        for switch in topo.get_all_switches():
            upports, downports = get_up_down_ports(topo, switch)
            for downport in downports:
                __run_ssh_remove_flow_command(switch, downport)
            for upport in upports:
                __run_ssh_remove_flow_command(switch, upport)


#####################################################
# Extra path for a path to Controller
#####################################################
CONTROLLER_IP_FOR_EXTRA_DEFAULT_PATH = "192.168.0.1"
def add_path_extra_for_controller(topo):
    ...
    controller_switch = topo.get_connected_switch(CONTROLLER_IP_FOR_EXTRA_DEFAULT_PATH)
    for switch in topo.get_all_nodes():
        if SDCNodeIdType.get_type(switch) == SDCNodeIdType.Edge:
            paths= networkx.all_shortest_paths(topo.topo_graph, 
                    source=switch, target=controller_switch, weight=None)
            path_list = list(paths)[0]
            for i in range(len(path_list) -1):
                extra_next_switch[str(path_list[i])] = str(path_list[i+1])
    for switch in extra_next_switch:
        outport = topo.get_switch_port_to_dst(switch, extra_next_switch[switch])
